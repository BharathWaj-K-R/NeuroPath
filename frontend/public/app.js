...
'</div>' +
  ...
  function normalize(p) {
    ...
    /* Ensure modules is array of proper objects */
    if (!Array.isArray(modules)) {
      modules = [];
    }
    
    /* Filter out invalid modules, ensure structure */
    modules = modules.filter(function (m) {
      return m && typeof m === "object" && m.title;
    }).map(function (m) {
      return {
        title: m.title || "Untitled",
        content: m.content || m.body || "",
        resources: Array.isArray(m.resources) ? m.resources : [],
        exercises: Array.isArray(m.exercises) ? m.exercises : []
      };
    });
    
    /* Fallback only if modules empty AND legacy content exists */
